...
namespace ProjectMercury
{
    ...
    public class ParticleEffect : ISupportDeepCopy<ParticleEffect>
    {
        ...
        public void Trigger(ref Vector3 position, ref BoundingFrustum frustum, bool checkNearFar=false)
        {
            var identity = Matrix.Identity;

            this.Trigger(ref position, ref frustum, ref identity, checkNearFar);
        }
        ...
        /// <param name="checkNearFar">Should the near/far planes be checked in the culling. Its faster not to do it and your camera and gamestyle may not need it</param>
        public void Trigger(ref Vector3 position, BoundingFrustum[] frustums, bool checkNearFar=false)
        {
            var identity = Matrix.Identity;

            this.Trigger(ref position, frustums, ref identity, checkNearFar);
        }

        internal void Trigger(ref Vector3 position, ref BoundingFrustum frustum, ref Matrix world, Boolean checkNearFar, Int32 index = 0)
        {
        ...
        internal void Trigger(ref Vector3 position, BoundingFrustum[] frustums, ref Matrix world,  bool checkNearFar, int index=0)
        {
